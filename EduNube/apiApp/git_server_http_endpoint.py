...
from EduNube.settings import GIT_HTTP_ENDPOINT_HOST
import requests
...
class DefaultConfigGitSrvHTTPEpConsumer(ConfigGitSrvHTTPEpConsumer):
    protocol = GIT_HTTP_ENDPOINT_HOST.get('protocol')
    host = GIT_HTTP_ENDPOINT_HOST.get('host')
    port = GIT_HTTP_ENDPOINT_HOST.get('port')
    token = GIT_HTTP_ENDPOINT_HOST.get('token')
...
# to test: set test to True and run "python manage.py -c "import apiApp.git_server_http_endpoint"
test = False
if test:
